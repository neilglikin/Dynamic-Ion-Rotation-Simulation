...
""" Identical to visualize_sim_animate_positionspace.py, except the angular coordinate is phase-locked to the potential. """
...
save = True
savefile = loadfile[:-4] + '_wavefn_rotframe.gif'
maxnFrames = 151
...
# Cancel the rotation
psi_rotfollow = np.zeros_like(psi)
for i in range(len(tvals)):
    phi_shift = phi - np.mod(theta_vals[i], 2*np.pi)
    psi_toswap_lefttoright = psi[phi_shift <  0, i]
    psi_toswap_righttoleft = psi[phi_shift >= 0, i]
    psi_rotfollow[:, i] = np.append(psi_toswap_righttoleft, psi_toswap_lefttoright)
...
def animate(i):
    ...
    global phi, psi_rotfollow, tvals, V0_vals, frot_vals, theta_vals, framelist
    
    i = i+70

    i = framelist[i]
    t, V0, frot, theta = tvals[i], V0_vals[i], frot_vals[i], theta_vals[i]
    line1.set_data(phi, psi_rotfollow.real[:,i])
    line2.set_data(phi, psi_rotfollow.imag[:,i])
    line3.set_data(phi, np.absolute(psi_rotfollow[:,i])**2)
    line4.set_data(phi, V0/2*(np.cos(2*phi)+1))
